...
func NewScope(params ScopeParams) (*Scope, error) {
	...
	computeService, err := service.NewService(subscriptionId)
	//session, err := session.NewSession(aws.NewConfig().WithRegion(clusterConfig.Region))
	if err != nil {
		return nil, errors.Errorf("failed to create aws session: %v", err)
	}

	networkService, err := service.NewService(subscriptionId)
	//session, err := session.NewSession(aws.NewConfig().WithRegion(clusterConfig.Region))
	if err != nil {
		return nil, errors.Errorf("failed to create aws session: %v", err)
	}	
	...
	if params.AzureClients.compute == nil {
		params.AzureClients.compte = computeService.VirtualMachineClient
	}

	if params.AzureClients.network == nil {
		params.AzureClients.network = networkService.VirtualNetworkClient
	}

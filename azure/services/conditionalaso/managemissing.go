...
import (
	...
	asoutil "sigs.k8s.io/cluster-api-provider-azure/util/aso"
...
func New[T genruntime.MetaObject](ctrlClient client.Client, clusterName string, owner client.Object) aso.Reconciler[T] {
	return &reconciler[T]{
		Client:      ctrlClient,
...
func (r *reconciler[T]) CreateOrUpdateResource(ctx context.Context, spec azure.ASOResourceSpecGetter[T], serviceName string) (T, error) {
	...
	parameters, err := PatchedParameters(ctx, r.Scheme(), spec, existing)
	if err != nil {
	...
	if t, ok := spec.(aso.TagsGetterSetter[T]); ok {
		if err := reconcileTags(t, existing, false, parameters); err != nil {
	...
	labels[clusterv1beta1.ClusterNameLabel] = r.clusterName
	...
	annotations[asoannotations.PerResourceSecret] = asoutil.GetASOSecretName(r.clusterName)
	...
	diff := cmp.Diff(nil, parameters)
	log.V(2).Info("creating or updating resource", "diff", diff)
	return r.createOrUpdateResource(ctx, existing, parameters, false, serviceName)
}

...
func TestParameters(t *testing.T) {
	testcases := []struct {
		...
		{
			name: "rebuild parameters for VM when Azure provisioning state is Failed",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
			...
			existing: armcompute.VirtualMachine{
				ID:       ptr.To("/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Compute/virtualMachines/my-vm"),
				Name:     ptr.To("my-vm"),
				Location: ptr.To("test-location"),
				Properties: &armcompute.VirtualMachineProperties{
					...
					NetworkProfile: &armcompute.NetworkProfile{
						NetworkInterfaces: []*armcompute.NetworkInterfaceReference{
							{
								ID: ptr.To("/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Network/networkInterfaces/my-nic"),
							},
						},
					},
				},
			...
			expect: func(g *WithT, result any) {
				g.Expect(result).To(BeAssignableToTypeOf(armcompute.VirtualMachine{}))
				g.Expect(result.(armcompute.VirtualMachine).Location).To(Equal(ptr.To("test-location")))
				g.Expect(result.(armcompute.VirtualMachine).Properties).ToNot(BeNil())
				g.Expect(result.(armcompute.VirtualMachine).Properties.HardwareProfile).ToNot(BeNil())
			},
			expectedError: "",
		},
		{
			name: "rebuild parameters for failed VM with NICs attached to load balancer",
			spec: &VMSpec{
				Name:          "my-failed-vm",
				Role:          infrav1.Node,
				...
				Location:      "eastus",
				NICIDs: []string{
					"/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Network/networkInterfaces/my-failed-vm-nic",
				},
				SSHKeyData: "fakesshpublickey",
				Size:       "Standard_D2v3",
				Zone:       "1",
			...
			existing: armcompute.VirtualMachine{
				ID:       ptr.To("/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Compute/virtualMachines/my-failed-vm"),
				Name:     ptr.To("my-failed-vm"),
				...
				Properties: &armcompute.VirtualMachineProperties{
					...
					NetworkProfile: &armcompute.NetworkProfile{
						NetworkInterfaces: []*armcompute.NetworkInterfaceReference{
							{
								ID: ptr.To("/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Network/networkInterfaces/my-failed-vm-nic"),
					...
					HardwareProfile: &armcompute.HardwareProfile{
						VMSize: ptr.To(armcompute.VirtualMachineSizeTypes("Standard_D2v3")),
					},
				},
			},
			expect: func(g *WithT, result any) {
				...
				// Verify VM location is set
				g.Expect(vm.Location).To(Equal(ptr.To("eastus")))

				// Verify VM properties are rebuilt
				g.Expect(vm.Properties).ToNot(BeNil())

				// Verify hardware profile is set correctly
				g.Expect(vm.Properties.HardwareProfile).ToNot(BeNil())
				g.Expect(vm.Properties.HardwareProfile.VMSize).To(Equal(ptr.To(armcompute.VirtualMachineSizeTypes("Standard_D2v3"))))

				// Verify network profile is rebuilt with NIC references
				g.Expect(vm.Properties.NetworkProfile).ToNot(BeNil())
				g.Expect(vm.Properties.NetworkProfile.NetworkInterfaces).To(HaveLen(1))
				g.Expect(vm.Properties.NetworkProfile.NetworkInterfaces[0].ID).To(Equal(ptr.To("/subscriptions/123/resourceGroups/my-rg/providers/Microsoft.Network/networkInterfaces/my-failed-vm-nic")))
				g.Expect(vm.Properties.NetworkProfile.NetworkInterfaces[0].Properties.Primary).To(Equal(ptr.To(true)))

				// Verify storage profile is rebuilt
				g.Expect(vm.Properties.StorageProfile).ToNot(BeNil())
				g.Expect(vm.Properties.StorageProfile.OSDisk).ToNot(BeNil())
				g.Expect(vm.Properties.StorageProfile.OSDisk.OSType).To(Equal(ptr.To(armcompute.OperatingSystemTypesLinux)))
				g.Expect(vm.Properties.StorageProfile.OSDisk.DiskSizeGB).To(Equal(ptr.To[int32](128)))

				// Verify OS profile is rebuilt
				g.Expect(vm.Properties.OSProfile).ToNot(BeNil())
				g.Expect(vm.Properties.OSProfile.LinuxConfiguration).ToNot(BeNil())

				// Verify zones are set
				g.Expect(vm.Zones).To(HaveLen(1))
				g.Expect(vm.Zones[0]).To(Equal(ptr.To("1")))
			},
			expectedError: "",
		},
		{
			name: "do not rebuild parameters for VM when Azure provisioning state is Succeeded",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
		...
		{
			name: "do not rebuild parameters for VM when Azure provisioning state is Creating",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
			...
			existing: armcompute.VirtualMachine{
				...
				Location: ptr.To("test-location"),
				Properties: &armcompute.VirtualMachineProperties{
					ProvisioningState: ptr.To("Creating"),
				},
			},
			expect: func(g *WithT, result any) {
				g.Expect(result).To(BeNil())
		...
		{
			name: "do not rebuild parameters for VM when Azure provisioning state is Updating",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
		...
		{
			name: "do not rebuild parameters for VM when Azure provisioning state is Deleting",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
		...
		{
			name: "do not rebuild parameters for VM when Azure provisioning state is nil",
			spec: &VMSpec{
				Name:       "my-vm",
				Role:       infrav1.Node,
				NICIDs:     []string{"my-nic"},
			...
			existing: armcompute.VirtualMachine{
				...
				Location:   ptr.To("test-location"),
				Properties: &armcompute.VirtualMachineProperties{
					// ProvisioningState is nil - treat as existing VM to avoid recreation
				},
			},
			expect: func(g *WithT, result any) {
				g.Expect(result).To(BeNil())

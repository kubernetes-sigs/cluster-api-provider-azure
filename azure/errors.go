...
// ResourceNotFound parses an error to check if it's a resource not found error (404).
func ResourceNotFound(err error) bool {
	return hasStatusCode(err, 404)
}

// ResourceConflict parses an error to check if it's a resource conflict error (409).

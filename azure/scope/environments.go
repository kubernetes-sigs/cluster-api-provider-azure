...
func EnvironmentFromName(name string) (Environment, error) {
	...
	if !ok {
		return env, fmt.Errorf("no cloud environment matching the name %q", name)
	}
...
// This function is particularlyq useful in the Hybrid Cloud model, where one must define their own
// endpoints.
...
func (settings EnvironmentSettings) GetSubscriptionID() string {
	return settings.Values["AZURE_SUBSCRIPTION_ID"]
}

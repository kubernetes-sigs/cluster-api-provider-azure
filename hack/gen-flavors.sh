...
cluster_resource_sets_dir="${root}/templates/test/ci/clusterresourcesets"
...
find "${ci_dir}"* -not -path "${cluster_resource_sets_dir}" -maxdepth 0 -type d -print0 | xargs -0 -I {} basename {} | grep -v patches | xargs -I {} sh -c "${kustomize} build --load-restrictor LoadRestrictionsNone --reorder none ${ci_dir}{} > ${ci_dir}cluster-template-{}.yaml"

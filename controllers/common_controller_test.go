...
var _ = Describe("CommonReconcilerBehaviors", func() {
	...
	It("should trigger reconciliation if cluster is unpaused", func() {
		...
		logPausedEntry := logentries.GenerateUpdatePausedClusterLogEntries(ec)
		// check to make sure the cluster has reconciled and is paused
		Eventually(logListener.GetEntries, test.DefaultEventualTimeout, 1*time.Second).Should(ContainElements(logPausedEntry))
		...
		Expect(testEnv.Update(ctx, cluster)).To(Succeed())
		logUpdatePausedEntries := logentries.GenerateUpdatePausedClusterLogEntries(ec)
		Eventually(logListener.GetEntries, test.DefaultEventualTimeout, 1*time.Second).Should(ContainElements(logUpdatePausedEntries))
	})

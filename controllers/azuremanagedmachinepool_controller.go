...
func (ammpr *AzureManagedMachinePoolReconciler) reconcileNormal(ctx context.Context, scope *scope.ManagedMachinePoolScope) (reconcile.Result, error) {
	...
	if err := svc.Reconcile(ctx); err != nil {
		...
		if conditions.IsTrue(scope.InfraMachinePool, string(infrav1.AgentPoolsReadyCondition)) {
			conditions.Set(scope.InfraMachinePool, metav1.Condition{Type: string(infrav1.AgentPoolsReadyCondition), Status: metav1.ConditionFalse, Reason: infrav1.FailedReason, Message: err.Error()})
		}

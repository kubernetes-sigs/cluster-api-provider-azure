...
func (acr *AzureClusterReconciler) reconcileNormal(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if err := acs.Reconcile(ctx); err != nil {
		...
		conditions.Set(azureCluster, metav1.Condition{Type: string(infrav1.NetworkInfrastructureReadyCondition), Status: metav1.ConditionFalse, Reason: infrav1.FailedReason, Message: fmt.Sprintf("%s", wrappedErr.Error())})
...
func (acr *AzureClusterReconciler) reconcileDelete(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if err := acs.Delete(ctx); err != nil {
		...
		conditions.Set(azureCluster, metav1.Condition{Type: string(infrav1.NetworkInfrastructureReadyCondition), Status: metav1.ConditionFalse, Reason: clusterv1beta1.DeletionFailedReason, Message: fmt.Sprintf("%s", err.Error())})
		return reconcile.Result{}, wrappedErr
	}

...
func validateAzureMachineSpec(spec infrav1.AzureMachineSpec) field.ErrorList {
	...
	if errs := ValidateNetwork(spec.SubnetName, spec.AcceleratedNetworking, spec.NetworkInterfaces, field.NewPath("networkInterfaces")); len(errs) > 0 { //nolint:staticcheck
		allErrs = append(allErrs, errs...)
	}

	if errs := ValidateSystemAssignedIdentityRole(spec.Identity, spec.RoleAssignmentName, spec.SystemAssignedIdentityRole, field.NewPath("systemAssignedIdentityRole")); len(errs) > 0 { //nolint:staticcheck
		allErrs = append(allErrs, errs...)
	}
...
func ValidateSystemAssignedIdentity(identityType infrav1.VMIdentity, oldIdentity, newIdentity string, fldPath *field.Path) field.ErrorList {
	...
	if identityType == infrav1.VMIdentitySystemAssigned {
		if err := uuid.Validate(newIdentity); err != nil {

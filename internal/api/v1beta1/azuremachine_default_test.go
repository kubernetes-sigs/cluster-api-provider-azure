...
func TestAzureMachineSpec_SetDefaultSSHPublicKey(t *testing.T) {
	...
	err := SetDefaultAzureMachineSpecSSHPublicKey(&publicKeyExistTest.machine.Spec)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(publicKeyExistTest.machine.Spec.SSHPublicKey).To(Equal(existingPublicKey))

	err = SetDefaultAzureMachineSpecSSHPublicKey(&publicKeyNotExistTest.machine.Spec)
	g.Expect(err).NotTo(HaveOccurred())
...
func TestAzureMachineSpec_SetIdentityDefaults(t *testing.T) {
	...
	SetDefaultAzureMachineSpecIdentity(&roleAssignmentExistTest.machine.Spec, fakeSubscriptionID)
	g.Expect(roleAssignmentExistTest.machine.Spec.SystemAssignedIdentityRole.Name).To(Equal(existingRoleAssignmentName))

	SetDefaultAzureMachineSpecIdentity(&notSystemAssignedTest.machine.Spec, fakeSubscriptionID)
	g.Expect(notSystemAssignedTest.machine.Spec.SystemAssignedIdentityRole.Name).To(BeEmpty())

	SetDefaultAzureMachineSpecIdentity(&systemAssignedIdentityRoleExistTest.machine.Spec, fakeSubscriptionID)
	...
	SetDefaultAzureMachineSpecIdentity(&deprecatedRoleAssignmentNameTest.machine.Spec, fakeSubscriptionID)
	...
	g.Expect(deprecatedRoleAssignmentNameTest.machine.Spec.RoleAssignmentName).To(BeEmpty()) //nolint:staticcheck

	SetDefaultAzureMachineSpecIdentity(&emptyTest.machine.Spec, fakeSubscriptionID)
	g.Expect(emptyTest.machine.Spec.SystemAssignedIdentityRole.Name).To(Not(BeEmpty()))
	g.Expect(uuid.Validate(emptyTest.machine.Spec.SystemAssignedIdentityRole.Name)).To(Succeed())
	...
	SetDefaultAzureMachineSpecIdentity(&bothDeprecatedRoleAssignmentNameAndSystemAssignedIdentityRoleTest.machine.Spec, fakeSubscriptionID)
	g.Expect(bothDeprecatedRoleAssignmentNameAndSystemAssignedIdentityRoleTest.machine.Spec.RoleAssignmentName).To(Not(BeEmpty())) //nolint:staticcheck
...
func TestAzureMachineSpec_SetSpotEvictionPolicyDefaults(t *testing.T) {
	...
	setDefaultAzureMachineSpecSpotEvictionPolicy(&spotVMOptionsExistTest.machine.Spec)
	g.Expect(spotVMOptionsExistTest.machine.Spec.SpotVMOptions.EvictionPolicy).To(Equal(&deallocatePolicy))

	setDefaultAzureMachineSpecSpotEvictionPolicy(&localDiffDiskSettingsExistTest.machine.Spec)
...
func TestAzureMachineSpec_SetDataDisksDefaults(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			machine := apifixtures.HardcodedAzureMachineWithSSHKey(apifixtures.GenerateSSHPublicKey(true))
			machine.Spec.DataDisks = tc.disks
			SetDefaultAzureMachineSpecDataDisks(&machine.Spec)
...
func TestAzureMachineSpec_SetNetworkInterfacesDefaults(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			g := NewWithT(t)
			SetDefaultAzureMachineSpecNetworkInterfaces(&tc.machine.Spec)

...
func TestDefaultSubnetTemplate(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			setDefaultAzureManagedControlPlaneTemplateSubnet(tc.controlPlaneTemplate)
			if !reflect.DeepEqual(tc.controlPlaneTemplate, tc.outputTemplate) {
				expected, _ := json.MarshalIndent(tc.outputTemplate, "", "\t")
				actual, _ := json.MarshalIndent(tc.controlPlaneTemplate, "", "\t")
				t.Errorf("Expected %s, got %s", string(expected), string(actual))
...
func TestSetDefault(t *testing.T) {
	...
	var s *Struct
	SetZeroPointerDefault(&s, &Struct{"hello"})
	g.Expect(s.name).To(Equal("hello"))
	SetZeroPointerDefault(&s, &Struct{"world"})
	g.Expect(s.name).To(Equal("hello"))

	r := &Struct{}
	SetZeroPointerDefault(&r, &Struct{"a name"})
	g.Expect(r.name).To(BeEmpty())
	SetZeroPointerDefault(&r.name, "hello")
	g.Expect(r.name).To(Equal("hello"))
	SetZeroPointerDefault(&r.name, "world")
	g.Expect(r.name).To(Equal("hello"))

	str := ""
	SetZeroPointerDefault(&str, "a string")
	g.Expect(str).To(Equal("a string"))
	SetZeroPointerDefault(&str, "another string")
	g.Expect(str).To(Equal("a string"))
}

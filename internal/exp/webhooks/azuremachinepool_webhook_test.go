...
func TestAzureMachinePool_ValidateCreate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "azuremachinepool with shared gallery image - missing subscription",
		...
		{
			name:    "azuremachinepool with no image details at all",
			amp:     createMachinePoolWithImage(&infrav1.Image{}),
			wantErr: true,
		},
		{
		...
		{
			name:    "azuremachinepool with wrong terminate notification - above maximum",
			amp:     createMachinePoolWithSharedImage("SUB123", "RG123", "NAME123", "GALLERY1", "1.0.0", ptr.To(35)),
			wantErr: true,
		},
		{
			name:    "azuremachinepool with wrong terminate notification - below minimum",
		...
		},
		{
			...
			amp:     createMachinePoolWithDiagnostics(infrav1.DisabledDiagnosticsStorage, nil),
			wantErr: false,
		},
		{
		...
		{
			name:    "azuremachinepool with managed diagnostics profile, but a storage account uri is set",
			amp:     createMachinePoolWithDiagnostics(infrav1.ManagedDiagnosticsStorage, &infrav1.UserManagedBootDiagnostics{StorageAccountURI: "https://fakeurl"}),
			wantErr: true,
		},
		{
			name:    "azuremachinepool with disabled diagnostics profile, but a storage account uri is set",
			amp:     createMachinePoolWithDiagnostics(infrav1.DisabledDiagnosticsStorage, &infrav1.UserManagedBootDiagnostics{StorageAccountURI: "https://fakeurl"}),
			wantErr: true,
		},
		{
...
func TestAzureMachinePool_ValidateUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "azuremachinepool network interfaces subnet changed with no deprecated subnetName set",
		...
		{
			name:    "azuremachinepool cannot set both networkInterfaces and deprecated subnetName on update",
			oldAMP:  createMachinePoolWithNetworkConfig("", []infrav1.NetworkInterface{{SubnetName: "testSubnet"}}),
			amp:     createMachinePoolWithNetworkConfig("subnet", []infrav1.NetworkInterface{{SubnetName: "testSubnet2"}}),
			wantErr: true,
		},
		{
			name:    "azuremachinepool cannot set both networkInterfaces and deprecated subnetName when subnetName was already set",
...
func TestAzureMachinePool_Default(t *testing.T) {
	...
	// Only the systemAssignedIdentityRole.name is already set; scope and definitionID should be
	// freshly generated while the existing name is preserved.
	...
	// name, scope, and definitionID are all already populated, so defaulting should leave them untouched.
	...
	// Both the deprecated roleAssignmentName and its replacement systemAssignedIdentityRole.name are already
	// populated with the same value, so defaulting should leave everything else untouched.
	bothRoleAssignmentNamesPopulatedTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Identity:           "SystemAssigned",
	...
	// Identity is not SystemAssigned, so no SystemAssignedIdentityRole defaulting should happen.
	notSystemAssignedTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
	...
	// Only the deprecated roleAssignmentName is set; defaulting should migrate it into
	// systemAssignedIdentityRole.name and clear the deprecated field.
	deprecatedRoleAssignmentNameTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Identity:           "SystemAssigned",
			RoleAssignmentName: existingRoleAssignmentName,
		},
	...
	nilDiagnosticsTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
	...
	nilBootDiagnosticsTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
	...
	// Diagnostics that are already fully specified should be left untouched by defaulting.
	managedDiagnosticsTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				Diagnostics: &infrav1.Diagnostics{
	...
	disabledDiagnosticsTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				Diagnostics: &infrav1.Diagnostics{
					Boot: &infrav1.BootDiagnostics{
	...
	userManagedDiagnosticsTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				Diagnostics: &infrav1.Diagnostics{
					Boot: &infrav1.BootDiagnostics{
	...
	deprecatedSubnetNameTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
	...
	deprecatedAcceleratedNetworkingTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				SubnetName:            "test-subnet",
	...
	bothNetworkFieldsSetTest := test{amp: &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				SubnetName: "test-subnet",
	...
	g.Expect(roleAssignmentExistTest.amp.Spec.SystemAssignedIdentityRole.Scope).To(Equal(fmt.Sprintf("/subscriptions/%s/", fakeSubscriptionID)))
	g.Expect(roleAssignmentExistTest.amp.Spec.SystemAssignedIdentityRole.DefinitionID).To(Equal(fmt.Sprintf("/subscriptions/%s/providers/Microsoft.Authorization/roleDefinitions/%s", fakeSubscriptionID, apiinternal.ContributorRoleID)))
	...
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(systemAssignedIdentityRoleExistTest.amp.Spec.SystemAssignedIdentityRole.Name).To(Equal(existingRoleAssignmentName))
	...
	err = ampw.Default(t.Context(), bothRoleAssignmentNamesPopulatedTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(bothRoleAssignmentNamesPopulatedTest.amp.Spec.RoleAssignmentName).To(Equal(existingRoleAssignmentName)) //nolint:staticcheck
	g.Expect(bothRoleAssignmentNamesPopulatedTest.amp.Spec.SystemAssignedIdentityRole.Name).To(Equal(existingRoleAssignmentName))
	g.Expect(bothRoleAssignmentNamesPopulatedTest.amp.Spec.SystemAssignedIdentityRole.Scope).To(BeEmpty())
	g.Expect(bothRoleAssignmentNamesPopulatedTest.amp.Spec.SystemAssignedIdentityRole.DefinitionID).To(BeEmpty())

	err = ampw.Default(t.Context(), notSystemAssignedTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(notSystemAssignedTest.amp.Spec.SystemAssignedIdentityRole).To(BeNil())

	err = ampw.Default(t.Context(), deprecatedRoleAssignmentNameTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(deprecatedRoleAssignmentNameTest.amp.Spec.RoleAssignmentName).To(BeEmpty()) //nolint:staticcheck
	g.Expect(deprecatedRoleAssignmentNameTest.amp.Spec.SystemAssignedIdentityRole.Name).To(Equal(existingRoleAssignmentName))
	g.Expect(deprecatedRoleAssignmentNameTest.amp.Spec.SystemAssignedIdentityRole.Scope).To(Equal(fmt.Sprintf("/subscriptions/%s/", fakeSubscriptionID)))
	g.Expect(deprecatedRoleAssignmentNameTest.amp.Spec.SystemAssignedIdentityRole.DefinitionID).To(Equal(fmt.Sprintf("/subscriptions/%s/providers/Microsoft.Authorization/roleDefinitions/%s", fakeSubscriptionID, apiinternal.ContributorRoleID)))

	err = ampw.Default(t.Context(), nilDiagnosticsTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(nilDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.StorageAccountType).To(Equal(infrav1.ManagedDiagnosticsStorage))

	err = ampw.Default(t.Context(), nilBootDiagnosticsTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(nilBootDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.StorageAccountType).To(Equal(infrav1.ManagedDiagnosticsStorage))

	// Diagnostics that are already fully specified should be left untouched by defaulting.
	err = ampw.Default(t.Context(), managedDiagnosticsTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(managedDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.StorageAccountType).To(Equal(infrav1.ManagedDiagnosticsStorage))

	err = ampw.Default(t.Context(), disabledDiagnosticsTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(disabledDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.StorageAccountType).To(Equal(infrav1.DisabledDiagnosticsStorage))

	err = ampw.Default(t.Context(), userManagedDiagnosticsTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(userManagedDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.StorageAccountType).To(Equal(infrav1.UserManagedDiagnosticsStorage))
	g.Expect(userManagedDiagnosticsTest.amp.Spec.Template.Diagnostics.Boot.UserManaged.StorageAccountURI).To(Equal("https://fakeurl"))

	// SetNetworkInterfacesDefaults migrates the deprecated subnetName/acceleratedNetworking fields into
	// NetworkInterfaces, and is a no-op when both the deprecated and replacement fields are already set.
	err = ampw.Default(t.Context(), deprecatedSubnetNameTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(deprecatedSubnetNameTest.amp.Spec.Template.SubnetName).To(BeEmpty()) //nolint:staticcheck
	g.Expect(deprecatedSubnetNameTest.amp.Spec.Template.NetworkInterfaces).To(Equal([]infrav1.NetworkInterface{
		{SubnetName: "test-subnet", PrivateIPConfigs: 1},
	}))

	err = ampw.Default(t.Context(), deprecatedAcceleratedNetworkingTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(deprecatedAcceleratedNetworkingTest.amp.Spec.Template.SubnetName).To(BeEmpty())          //nolint:staticcheck
	g.Expect(deprecatedAcceleratedNetworkingTest.amp.Spec.Template.AcceleratedNetworking).To(BeNil()) //nolint:staticcheck
	g.Expect(deprecatedAcceleratedNetworkingTest.amp.Spec.Template.NetworkInterfaces).To(Equal([]infrav1.NetworkInterface{
		{SubnetName: "test-subnet", PrivateIPConfigs: 1, AcceleratedNetworking: ptr.To(true)},
	}))

	err = ampw.Default(t.Context(), bothNetworkFieldsSetTest.amp)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(bothNetworkFieldsSetTest.amp.Spec.Template.SubnetName).To(Equal("test-subnet")) //nolint:staticcheck
	g.Expect(bothNetworkFieldsSetTest.amp.Spec.Template.NetworkInterfaces).To(Equal([]infrav1.NetworkInterface{
		{SubnetName: "test-subnet"},
	}))
}
...
func createMachinePoolWithImage(image *infrav1.Image) *infrav1exp.AzureMachinePool {
	return &infrav1exp.AzureMachinePool{
		Spec: infrav1exp.AzureMachinePoolSpec{
			Template: infrav1exp.AzureMachinePoolMachineTemplate{
				Image:        image,
...
func createMachinePoolWithUserAssignedIdentity(providerIDs []string) *infrav1exp.AzureMachinePool {
	userAssignedIdentities := make([]infrav1.UserAssignedIdentity, 0, len(providerIDs))

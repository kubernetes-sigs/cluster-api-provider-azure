...
To run E2E locally, set `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `AZURE_SUBSCRIPTION_ID`, `AZURE_TENANT_ID`, (`LOCAL_ONLY` or `REGISTRY`) and run:
...
| `LOCAL_ONLY`               | Only use Kind local registry and run the subset of tests which don't require a remotely pushed controller image.          | `false`                                                                               |
| `REGISTRY`                 | Registry to push the controller image. If `LOCAL_ONLY=true`, this is optional.                                            | `capzci.azurecr.io/ci-e2e`                                                            |

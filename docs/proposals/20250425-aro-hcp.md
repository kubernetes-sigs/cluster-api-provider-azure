...
type PlatformProfile struct {
  // Location should be valied Azure location ex; centralus
  Location string `json:"location,omitempty"`

  // Azure ManagedResourceGroup id
...
type AROControlPlaneStatus struct {
  // Initialized indicate whether or not the control plane has initialized. 
  Initialized bool `json:"initialized"`

  // Ready indicate that the AROControlPlane API Server is ready to receive requests.
...
spec:
  ...
  platform:
    location: eastus
    managedResourceGroup: "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/aro-rg"
    subnetId: "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/aro-vnet-rg/providers/Microsoft.Network/virtualNetworks/aro-vnet/subnets/aro-subnet"
...
type AROMachinePoolSpec struct {
  ...
  // OpenShift semantic version, for example "4.20.0". Version specifies the OpenShift version of the nodes associated with this machinepool.
...
// PlatformProfile represents the Azure platform configuration.
type PlatformProfile struct {
  // Subnet Id where the nodePool will be attached to it.
  SubnetID string `json:"subnetId,omitempty"`

  // VMSize set the VM disk volume size the node.
  VMSize string `json:"vmSize,omitempty"`

  // DiskSizeGiB set the disk volume size for the machine pool, in Gib.
...
type AROMachinePoolStatus struct {
  ...
  // Ready indicate that the AROMachinePool (nodePool) is joined the ARO-HCP cluster and ready to deploy workload.
  Ready bool `json:"ready"`
...
```yaml
apiVersion: controlplane.openshift.io/v1alpha1
kind: AROMachinePool
metadata:
...
spec:
  ...
  version: "4.20.0"
  platform:
    subnetId: "subnets/worker-subnet"
...
type AROClusterStatus struct {
  ...
  // Conditions defines current service state of the AROCluster.
  Conditions clusterv1.Conditions `json:"conditions,omitempty"`
}
```

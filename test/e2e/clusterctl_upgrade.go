...
// This file is a near-verbatim copy of ClusterctlUpgradeSpec from
// sigs.k8s.io/cluster-api/test/e2e/clusterctl_upgrade.go (v1.13.2), vendored into
// CAPZ so we can stand up a highly-available (multi control-plane) self-hosted
// management cluster during the API version upgrade test. The upstream spec
// hardcodes a single management control-plane machine, which leaves the public
// API server load balancer with one backend and makes the apiversion-upgrade job
// flaky under upgrade load.
//
// The only functional change from upstream is the new
// ManagementClusterControlPlaneMachineCount input field (and its use when
// creating the management cluster); search for "CAPZ ADDITION". Once that field
// is accepted upstream, this copy should be deleted and the callers switched back
// to capi_e2e.ClusterctlUpgradeSpec.
...
type ClusterctlUpgradeSpecInput struct {
	...
	// ManagementClusterControlPlaneMachineCount specifies the number of control plane machines to create in the
	// self-hosted management cluster that this spec stands up (the cluster the providers are upgraded on).
	// If not set, it defaults to 1.
	//
	// CAPZ ADDITION (not yet in upstream cluster-api): the upstream spec hardcodes a single control plane machine
	// for the management cluster, which leaves its public API server load balancer with a single backend. Under the
	// load of `clusterctl upgrade apply` that single node can briefly become unreachable, flaking the test. Allowing
	// an HA management control plane mitigates that. This field is intended to be cherry-picked upstream.
...
func ClusterctlUpgradeSpec(ctx context.Context, inputGetter func() ClusterctlUpgradeSpecInput) {
	...
	It("Should create a management cluster and then upgrade all the providers", func() {
		...
		if input.UseKindForManagementCluster {
			...
			// CAPZ ADDITION (not yet in upstream cluster-api): allow the management cluster to be HA.
			managementClusterControlPlaneMachineCount := ptr.To[int64](1)
			if input.ManagementClusterControlPlaneMachineCount != nil {
				managementClusterControlPlaneMachineCount = input.ManagementClusterControlPlaneMachineCount
			}
			...
			clusterctl.ApplyClusterTemplateAndWait(ctx, clusterctl.ApplyClusterTemplateAndWaitInput{
				...
				ConfigCluster: clusterctl.ConfigClusterInput{
					...
					KubernetesVersion:        initKubernetesVersion,
					ControlPlaneMachineCount: managementClusterControlPlaneMachineCount,
		...
		if input.PostNamespaceCreated != nil {
			Logf("Calling postNamespaceCreated for namespace %s", testNamespace.Name)
		...
		Logf("Applying the cluster template yaml to the cluster in dry-run")
		...
		for i, upgrade := range input.Upgrades {
			...
			if upgrade.WithBinary != "" {
				...
				defer func() { _ = os.Remove(clusterctlBinaryPath) }() //nolint:gocritic // Resource leakage is not a concern here.
			}
...
func validateMachineRollout(preMachineList, postMachineList *unstructured.UnstructuredList) bool {
	...
	Logf("Rollout detected (%d Machines before, %d Machines after)", len(preMachineList.Items), len(postMachineList.Items))
...
// package and are reproduced here because they are referenced by the vendored
// ClusterctlUpgradeSpec above but are not otherwise exported. They should be removed
// along with the rest of this file once the upstream change lands.

const (
	// KubernetesVersion is the env var key for the Kubernetes version used by the spec.
	KubernetesVersion = "KUBERNETES_VERSION"
	// IPFamily is the env var key for the IP family used by the spec.
	IPFamily = "IP_FAMILY"
)

// dumpKindClusterLogs collects logs from the nodes of a kind management cluster.
// Copied from sigs.k8s.io/cluster-api/test/e2e/common.go.
func dumpKindClusterLogs(ctx context.Context, artifactFolder string, clusterProxy framework.ClusterProxy) {
	...
	for i := range nodes.Items {
		...
		err = clusterLogCollector.CollectMachineLog(
			...
			// The kind cluster is not a CAPI cluster, so in order to reuse the logCollector,

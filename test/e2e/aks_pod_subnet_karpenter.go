...
const (
	...
	karpenterNodePoolName       = "pod-subnet"
	karpenterNodeClassName      = "default"
...
//
// The regression it guards against produces Karpenter nodes whose pods hold node subnet IPs while
// still reporting Running, so the assertions are on the pod IP CIDR rather than on pod phase.
func AKSPodSubnetKarpenterSpec(ctx context.Context, inputGetter func() AKSPodSubnetKarpenterSpecInput) {
	...
	prefixes := parsePodSubnetPrefixes(input.NodeSubnetCIDR, input.PodSubnetCIDR)
	...
	// These are created here rather than in the cluster template because CAPZ's controller has
	// no RBAC for the managedidentity.azure.com or authorization.azure.com API groups. Listing
	// them in AzureASOManagedControlPlane.spec.resources makes CAPZ's ResourceReconciler fail to
	// watch them, which wedges the control plane and leaves the cluster unprovisioned. ASO itself
	// has the necessary RBAC, so creating them directly works.
	...
	values := buildKarpenterValues(ctx, input, clusterProxy)
	...
	By("creating a Karpenter NodePool and AKSNodeClass")
	dynamicClient := newDynamicClient(clusterProxy)
	createKarpenterNodePool(ctx, dynamicClient)
	// Karpenter provisions standalone VMs and NICs in the node resource group but attaches them
	// to subnets ASO owns in the cluster resource group. Neither AKS nor ASO knows those NICs
	// exist, and cluster teardown deletes the two groups without ordering between them, so a
	// leftover NIC can block the VNet delete with InUseSubnetCannotBeDeleted. Removing the
	// NodePool here makes Karpenter drain its own VMs while its controller is still running.
	defer deleteKarpenterNodePool(ctx, dynamicClient, input.WaitIntervals)

	By("scheduling a workload that only a Karpenter node can satisfy")
	// Selecting on the NodePool label means no existing AKS node can run this pod, so
	// Karpenter must provision one. That is deterministic, unlike relying on resource pressure.
	nodeSelector := map[string]string{karpenterNodePoolLabel: karpenterNodePoolName}
	createPodSubnetProbeDeployment(ctx, clusterProxy, corev1.NamespaceDefault, podSubnetProbeDeploymentName, specName, nodeSelector)
	defer deletePodSubnetProbeDeployment(ctx, clientset, corev1.NamespaceDefault, podSubnetProbeDeploymentName)

	By("waiting for Karpenter to provision a node")
	karpenterNodes := waitForKarpenterNodes(ctx, clusterProxy, input.WaitIntervals)
	Expect(karpenterNodes).NotTo(BeEmpty(), "expected Karpenter to provision at least one node")
	Logf("Karpenter provisioned nodes: %s", strings.Join(karpenterNodes, ", "))

	nodeNames := nodeNameSet(karpenterNodes...)

	By("verifying the Karpenter node has an IP from the node subnet")
	expectNodeIPsInNodeSubnet(ctx, clientset, prefixes, nodeNames)

	By("verifying pods on the Karpenter node have IPs from the pod subnet")
	expectPodIPsInPodSubnet(ctx, clientset, prefixes, nodeNames)
}
...
// assignments granting it the Azure permissions it needs.
//
// The role assignments are scoped to the node resource group, where Karpenter creates VMs and
// NICs, plus the virtual network, which lives in the cluster resource group and so is not covered
// by the node resource group scope.
...
// ConfigMaps that other resources publish, so ASO retries them until those become available.
...
// the provider's hack/deploy/configure-values.sh but sourcing everything from the Azure SDK and
// the clusters directly rather than shelling out to az.
func buildKarpenterValues(ctx context.Context, input AKSPodSubnetKarpenterSpecInput, clusterProxy framework.ClusterProxy) []string {
	...
	pool := mc.Properties.AgentPoolProfiles[0]
	...
	// There is no StaticBlock check here on purpose: the containerservice v1api20240901 API
	// version this flavor pins has no podIpAllocationMode field, so pools created through it
	...
	// Passed as --set flags rather than a values file, matching how the other Helm-based
	// specs configure their charts. Indices are explicit because the chart appends
	// controller.env verbatim to the container's env list.
	...
	// --set infers types, so a value like "false" becomes a YAML boolean. Kubernetes requires
	// strings for label values and for EnvVar.value, and the chart renders both with a bare
	// toYaml, so anything that must stay a string uses --set-string. replicas and the feature
	// gate are genuinely numeric and boolean, so they keep --set.
	args := []string{
		"--set", "replicas=1",
...
// escapeHelmValue escapes the characters Helm's --set parser treats as structure, so values
// like comma-separated resource IDs survive intact. --set-string infers no types but uses the
// same structural parser, so the escaping is still required. Replacer makes a single pass, so
// the backslashes introduced here are not escaped again.
...
// createKarpenterNodePool creates the NodePool and AKSNodeClass Karpenter provisions from.
func createKarpenterNodePool(ctx context.Context, dynamicClient dynamic.Interface) {
	...
	_, err := dynamicClient.Resource(karpenterNodeClassGVR).Create(ctx, nodeClass, metav1.CreateOptions{})
	Expect(err).NotTo(HaveOccurred())
...
// waitForKarpenterNodes waits until at least one Karpenter-provisioned node is registered and
// ready, and returns the names of those nodes.
// deleteKarpenterNodePool removes the NodePool and waits for Karpenter to finish reclaiming what
// it provisioned. Deleting the NodePool cascades to its NodeClaims via owner references, and each
// NodeClaim is only removed once Karpenter has deleted the backing VM and NIC.
//
// This waits on NodeClaims rather than Nodes on purpose: if the spec fails while an instance is
// still coming up, no Node has registered yet, so waiting on Nodes would return immediately and
// leave the VM behind.
func deleteKarpenterNodePool(ctx context.Context, dynamicClient dynamic.Interface, intervals []interface{}) {
	By("deleting the Karpenter NodePool so Karpenter reclaims the VMs it created")
	if err := dynamicClient.Resource(karpenterNodePoolGVR).Delete(ctx, karpenterNodePoolName, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		// Returning here would leave the VMs for cluster teardown to trip over, so fail loudly.
		Expect(err).NotTo(HaveOccurred(), "failed to delete Karpenter NodePool %s", karpenterNodePoolName)
	}

	Eventually(func(g Gomega) {
		nodeClaims, err := dynamicClient.Resource(karpenterNodeClaimGVR).List(ctx, metav1.ListOptions{
			LabelSelector: fmt.Sprintf("%s=%s", karpenterNodePoolLabel, karpenterNodePoolName),
		})
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(nodeClaims.Items).To(BeEmpty(), "Karpenter has not finished reclaiming its instances yet")
	}, intervals...).Should(Succeed())

	if err := dynamicClient.Resource(karpenterNodeClassGVR).Delete(ctx, karpenterNodeClassName, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		Logf("failed to delete Karpenter AKSNodeClass %s: %v", karpenterNodeClassName, err)
	}
}

func waitForKarpenterNodes(ctx context.Context, clusterProxy framework.ClusterProxy, intervals []interface{}) []string {
	var names []string
	Eventually(func(g Gomega) {
		...
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(nodes.Items).NotTo(BeEmpty(), "no Karpenter node has registered yet")
...
// parsePodSubnetPrefixes parses the node and pod subnet CIDRs and asserts they don't
// overlap. Overlapping prefixes would make the pod subnet assertions meaningless, since
// a node subnet IP would also satisfy the pod subnet check.
func parsePodSubnetPrefixes(nodeCIDR, podCIDR string) podSubnetPrefixes {
	...
	// Overlapping prefixes would make the pod subnet assertions meaningless, since a node
	// subnet IP would also satisfy the pod subnet check.
	Expect(node.Contains(pod.IP) || pod.Contains(node.IP)).To(BeFalse(),
		"node subnet %s and pod subnet %s must not overlap", node, pod)

	return podSubnetPrefixes{node: node, pod: pod}
}
...
// expectPodIPsInPodSubnet asserts every running pod that uses the cluster network holds an
// IP from the pod subnet and not from the node subnet. When nodeNames is empty pods on all
// nodes are checked, otherwise only pods scheduled onto the named nodes.
//
// The failure this guards against produces healthy Running pods holding node subnet IPs, so
// asserting on phase alone would not catch it.
func expectPodIPsInPodSubnet(ctx context.Context, clientset kubernetes.Interface, prefixes podSubnetPrefixes, nodeNames map[string]struct{}) {
	...
	for _, pod := range pods.Items {
		...
		Expect(prefixes.pod.Contains(ip)).To(BeTrue(),
			"pod %s/%s on node %s has IP %s outside the pod subnet %s",
			pod.Namespace, pod.Name, pod.Spec.NodeName, pod.Status.PodIP, prefixes.pod)
		Expect(prefixes.node.Contains(ip)).To(BeFalse(),
			"pod %s/%s on node %s has IP %s from the node subnet %s, which means pod IPAM fell back to the node subnet",
			pod.Namespace, pod.Name, pod.Spec.NodeName, pod.Status.PodIP, prefixes.node)
		checked++
	}
	Expect(checked).To(BeNumerically(">", 0), "expected to check at least one cluster-networked pod")
}

// createPodSubnetProbeDeployment schedules a workload that uses the cluster network and
// waits for it to become available. nodeSelector may be nil to let it land anywhere.
func createPodSubnetProbeDeployment(ctx context.Context, clusterProxy framework.ClusterProxy, namespace, name, specName string, nodeSelector map[string]string) {
...
// deletePodSubnetProbeDeployment removes the probe workload, tolerating an already deleted one.
func deletePodSubnetProbeDeployment(ctx context.Context, clientset kubernetes.Interface, namespace, name string) {
	Byf("deleting deployment %s/%s", namespace, name)
	if err := clientset.AppsV1().Deployments(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
		Logf("failed to delete deployment %s/%s: %v", namespace, name, err)
	}
}
...
//
// This runs before the cluster template is applied rather than being left to the template alone.
// AKS validates that the subnets exist when the ManagedCluster is created, but the ManagedCluster
// and the subnets live on different CAPZ objects reconciled by different controllers, so nothing
// orders them. Losing that race is not self-correcting: AKS returns SubnetNotFound, which ASO
// classifies as RetryVerySlow and so does not retry for a full sync period (an hour by default),
// far longer than the test waits for the cluster to provision.
//
// The template still declares these same resources. CAPZ applies them server-side with
// ForceOwnership, so it adopts what is already here instead of conflicting, and by then they are
// provisioned and the ManagedCluster's subnet references resolve immediately.
func CreatePodSubnetNetwork(ctx context.Context, input PodSubnetNetworkInput) {
	...
	// Azure serializes writes to the subnets of one virtual network, so these are created one at a
	// time. Creating both at once makes the second fail with AnotherOperationInProgress, which is
	// retryable but costs a backoff for no reason.
	subnets := []*asonetworkv1.VirtualNetworksSubnet{
		newPodSubnetSubnet(fmt.Sprintf("%s-node-subnet", input.ClusterName), ns, "node-subnet", vnetName, input.NodeSubnetCIDR, credentialFrom),
		newPodSubnetSubnet(fmt.Sprintf("%s-pod-subnet", input.ClusterName), ns, "pod-subnet", vnetName, input.PodSubnetCIDR, credentialFrom),
	}
